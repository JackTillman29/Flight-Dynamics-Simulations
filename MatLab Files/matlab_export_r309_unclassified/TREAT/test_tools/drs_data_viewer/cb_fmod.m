...
%for k = 1 : length(haxes)
    ...
    input_res = inputdlg('Modulation Frequency (Hz)','?');
    fmod = eval(input_res{1});
    ...
    figure;
    subplot(2,1,1);
    plot(angle(v1.*exp(1i*2*pi*fmod.*(0:length(v1)-1)'.*ts)));
    xlabel('Sample');ylabel('rad');
    subplot(2,1,2);
    plot(angle(v2.*exp(1i*2*pi*fmod.*(0:length(v2)-1)'.*ts)));
    xlabel('Sample');ylabel('rad');
