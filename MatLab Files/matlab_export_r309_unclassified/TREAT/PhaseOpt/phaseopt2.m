...
figure('position',[520 121 996 677]); add_print_callbacks;
haxhp1 = subplot(2,1,1)
hp = plot(0,0);

% yfcn   = @(f,p,t) sum(cos(2*pi*f'*t + p'),1);
yfcn   = @(f,p,t) sum(exp(1i*2*pi*f'*t + 1i*p'),1);

% fv = [.5:0.5:2.5]*1000;
% fv = [-2000 -1000 1000 2000];
fv = [-2000 -1000 0 1000 2000];
fv = [-2000 -1000 0 1000 2000];
% fv = [1000 2000 3000 4000];
% fv = [1000 2000];
...
%     pv = [0 0 0 0 0];
    ...
    yout = yfcn(fv,pv,t);
    set(hp,'XData',t,'YData',real(yout));
%     yrms = sqrt(mean((yout./max(abs(yout))).^2));
    yrms = sqrt(mean((abs(yout)./max(abs(yout))).^2));
...
%     set( hp2,'XData',farr,'YData',10*log10(abs(fyout)) )
    ...
    for k = 1 : length(fv)
        ...
        yout2(k) = max(abs(yfcn(fv,pv2,t))) - p;
    end
    ...
    totalLoss_dB(kopt) = 20*log10(1./max(abs(yout)));
    
    if(best_case < totalLoss_dB(kopt))
        best_case = totalLoss_dB(kopt);
        best_case_k = kopt;
        best_case_phs = pv;
    end
