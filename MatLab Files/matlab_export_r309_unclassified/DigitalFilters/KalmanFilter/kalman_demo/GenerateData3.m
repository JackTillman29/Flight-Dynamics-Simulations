function[t,trueData,measData] = GenerateData3(xi,yi,xvi,yvi,siteX,siteY,stdRange,stdAngle,dt,dragw)
...
for k = 2:length(t)
   % fpa  = atan2(trueData.velocity_y(k-1),trueData.velocity_x(k-1));  %
   % flight path angle
    dragx = -trueData.velocity_x(k-1)^2 * dragw;
    dragy =  - 9.80665;
...
if(dragw ~= 0)  % might need to recheck length of data
