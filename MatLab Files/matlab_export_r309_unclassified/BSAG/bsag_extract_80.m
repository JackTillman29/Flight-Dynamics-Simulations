...
% firstPacket = 653665483 ;
% lastPacket  = 653886983 ; 
...
triggering.pulserise_indiv_pre   = floor(8e-6 / dt_data);
triggering.pulserise_indiv_post  = floor(8e-6 / dt_data);
...
%for k = 1 : 1
  %  lastPacket = firstPacket-1+nPackets
   ...
   % xm2_TG = int32(x_TG(1:2:end)).^2+int32(x_TG(2:2:end)).^2;
    ...
    imData_TG = 0*imData;
    imData_TGIQ = complex(zeros(triggers_pulse.trail-triggers_pulse.lead+1,length(triggers_pulse.lead),'single'))';
        ...
        % generate timing jitter in data post-test
         ...
         for imRow = 1 : size(imData,1)
            ...
            idx_offset = 0; %GenerateTimingJitter(size(imData,1),floor(0.5e-6/dt_data));
            
            imData(imRow,:) = single(xm2(idx_start:idx_stop));
            imData_TG(imRow,:) = single(xm2_TG((idx_start+idx_offset):(idx_stop+idx_offset)));
            imData_TGIQ(imRow,:) = single(xm2_TGI((idx_start+idx_offset):(idx_stop+idx_offset))) + 1i*single(xm2_TGQ((idx_start+idx_offset):(idx_stop+idx_offset)));
        end
