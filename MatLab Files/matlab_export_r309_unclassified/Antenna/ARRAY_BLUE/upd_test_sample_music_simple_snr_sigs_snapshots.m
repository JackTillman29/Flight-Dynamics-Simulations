...
nSamplesArray = logspace(1,4,200);

res.peak = 0*nSamplesArray;
res.bottom = 0*nSamplesArray;
res.nSamples = 0*nSamplesArray;

for ktrade = 1 : length(nSamplesArray)
nSamples = nSamplesArray(ktrade);
...
JNR_dB = 0; % dB at the element level
JNR = 10^(-JNR_dB/20);
...
res.peak(ktrade) = max(MUSIC_1D);
res.bottom(ktrade) = min(MUSIC_1D);
res.nSamples(ktrade) = nSamples;
close all;
