...
figure;
hs1=subplot(1,2,1);
plot(t,[real(y)' ]);co = get(gca,'ColorOrder');
xlim([-0.1 1.1]);
ylim([-1.1 1.1]);
...
%hpQ2 = plot(t(k),-imag(y(k)));set(hpQ2,'Marker','o','MarkerFaceColor',co(2,:),'MarkerEdgeColor','none');
grid on;
...
hpQ3 = plot(0,imag(y(k)));set(hpQ3,'Marker','o','MarkerFaceColor',co(2,:),'MarkerEdgeColor','none');
...
hLineVert2 = line([real(y(k)) real(y(k))],[0 imag(y(k))]);
hLineQ2 = line([real(y(k)) real(y(k))],[0 -imag(y(k))]);
hLineHorz = line([0 real(y(k))],[imag(y(k)) imag(y(k))]);
hLineHorz2 = line([0 real(y(k))],[imag(y(k)) imag(y(k))]);
...
set([hLineQ hLineQ2],'Color',co(2,:),'LineWidth',2);
xlabel('Real Axis');
ylabel('Imaginary Axis');
PrepForPrint(gcf,pp);
set([hLineVert hLineHorz hLineVert2 hLineHorz2],'Color','k','LineStyle',':','LineWidth',1);
