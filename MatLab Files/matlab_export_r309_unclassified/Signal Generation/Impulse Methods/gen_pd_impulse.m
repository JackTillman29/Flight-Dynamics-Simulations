function [u,varargout] = gen_pd_impulse(u,rng,prf,Ts,amp,LO,RF,doppler)
...
tTimeVec = ((first_rng_sample:arng_samples:length(u))-1) * Ts;
...
tPhaseVec = exp(1i*(2*pi*(LO+doppler)*tTimeVec));

% apply sampling phase error
tPhaseVec = exp(1i*phase_error) .* tPhaseVec;

% Sum these impulse terms with whatever was there prior
u(first_rng_sample:arng_samples:end) = ...
    u(first_rng_sample:arng_samples:end) + amp .* ...
      tPhaseVec;
