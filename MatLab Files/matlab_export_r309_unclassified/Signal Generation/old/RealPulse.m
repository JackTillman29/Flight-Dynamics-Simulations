classdef RealPulse < hgsetget
    ...
    methods
        function p = RealPulse(varargin)
            
            if (nargin == 1 & isa(varargin{1},'RealPulse'))
        ...
        function generateTDSignal(p)
            ...
            p.amplitudeVector = p.windowfcn(length(p.timeVector))' .* p.peakAmplitude .* sin(2*pi*p.carrier*p.timeVector);
        end
        ...
        function plot(this)
            ...
            subplot(2,1,1);
            plot(1e6*this.timeVector,this.amplitudeVector);
            axis tight;
            grid on;
            xlabel('Time (\musec)');
            ylabel('Amplitude');
        ...
        function pp = partialPulse(p,fctStart,fctStop)
            iStart = round(length(p.timeVector) * fctStart);
        ...
        function [A,fv,tv]=ambiguityFunction(p,pwpct,pwpts,frqpct,frqpts)
            if ( nargin == 1 )
                disp('Using defaults for: pwpct,pwpts,frqpct,frqpts');
                pwpct = 1.0;
                pwpts = 10;
                frqpct= 1.0;
                frqpts = 10;
            ...
            for kfreq = 1:frqpts
                ...
                crossa = PeakDetect(crossa,crossk);
            ...
            if ( nargout == 0 )
                ...
                surf(1e6*tv,1e-3*fv,abs(A),'LineStyle','none');
