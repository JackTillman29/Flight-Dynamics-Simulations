function cb_frq(varargin)
    ...
    % next determine if there is an xlabel, we will use this
    % to scale the x points back into "sec"
    hax_xlabel = get(gca,'XLabel');
    xlab_str = get(hax_xlabel,'String');
    no_x_label = 0;
    if(strcmp(xlab_str,''))
        no_x_label = 1;
        unitstr = '';
    else
        idx1 = strfind(xlab_str,'('); idx2 = strfind(xlab_str,')');
        ...
        if(isempty(idx1) | isempty(idx2))
            ...
            unitstr = xlab_str(idx1+1:idx2-1);
        end
%         compute_freq(dx,unitstr)
    end

    if(no_x_label)
    % no xlabel (e.g. rookie analyst for sure), time to go rogue....
    % check if there is a variable "Fs" in the workspace
    try
        warning('No xlabel?? Results are inaccurate if x is not samples!!!');
        Fs = evalin('base','Fs');
    catch
        try
            warning('No Fs variable defined in workspace. Attempting Ts');
            Ts = evalin('base','Ts');
            Fs = 1.0 / Ts;
        catch
            error('No Ts variable defined in workspace. Please set Fs=<value> in workspace!');
        end
    end
    end
...
%     b = pres(2);
    ...
    set(hLine,'Color',linecolor,'LineWidth',2);
    
    [Freq,unit,unitt] = prepare_freq_display(dx,unitstr);
    
    hold on;
    ud.hTextBG = patch([0 1 1 0],[0 0 1 1],'w','Visible','off','FaceAlpha',tb_facealpha,'EdgeColor','none');
    hold off;
    hText = text(0.5*(x1+x2),y1,{[sprintf(' %4.4g ',Freq) unit],[sprintf(' %4.4g ',1000.0./Freq) unitt]});
    set(hText,'Color',linecolor,'FontWeight','normal','BackgroundColor','none',...
    ...
    % =====================================================================
    % =====================================================================
    function [Freq,unit,unitt] = prepare_freq_display(dx,unitstr)
        
        if(strcmp(unitstr,''))
            no_x_label = 1;
        else
            
            if(strcmpi(unitstr,'s'))
                dx = dx * 1.0;
            elseif(strcmpi(unitstr,'ms'))
                dx = dx * 1.0e-3;
            elseif(strcmpi(unitstr,'us'))
                dx = dx * 1.0e-6;
            elseif(strcmpi(unitstr,'\mus'))
                dx = dx * 1.0e-6;
            elseif(strcmpi(unitstr,'ns'))
                dx = dx * 1.0e-9;
            elseif(strcmpi(unitstr,'ps'))
                dx = dx * 1.0e-12;
            else
                error('Improper time units!! Must be (s)(ms)(us)(ns)(ps)');
                return;
            end
            no_x_label = 0;
        end
    ...
    function update_textbox(varargin)
        ...
        set(ud.hText,'Position',[newc_axe(1) newc_axe(2)], ...
            'String',{[sprintf(' %4.4g ',Freq) unit],[sprintf(' %4.4g ',1000.0./Freq) unitt]});
    ...
    function textbox_move(varargin)
        ...
        set(htb,'Position',[cpt(1,1) cpt(1,2) 0])
